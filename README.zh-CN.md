...
- 两个通道、声音与内容预览默认开启。
- **界面语言跟随 DSH**：页面与通知文字按 DSH 选定的语言显示繁体中文或英文，切换语言后页面立即重新标注，Windows 悬浮卡片同样跟随。插件配置行的 `language` 字段可覆盖：默认 `auto` 跟随 DSH，尚未选过语言时使用繁体中文；`zh` 或 `en` 则固定主机生成的标题语言。已写入历史的通知保留当时的语言。原生声音使用 Windows 系统音量；浏览器声音在用户交互后播放本地合成的提示音，不下载远程音频。
